...
from spotify_python_sdk.model.artist_object import ArtistObject as ArtistObjectSchema
...
from spotify_python_sdk.pydantic.artist_object import ArtistObject as ArtistObjectPydantic
...
SchemaFor200ResponseBodyApplicationJson = ArtistObjectSchema
...
class GetCatalogInfo(BaseApi):
    ...
    async def aget_catalog_info(
        ...
        validate: bool = False,
        **kwargs,
    ) -> ArtistObjectPydantic:
        raw_response = await self.raw.aget_catalog_info(
            id=id,
            **kwargs,
        )
        if validate:
            return ArtistObjectPydantic(**raw_response.body)
        return api_client.construct_model_instance(ArtistObjectPydantic, raw_response.body)
